...
router.post('/signup', async (req, res) => {
  ...
  try {
    ...
    const roleMap = { jmo: 'jmo', doctor: 'doctor', lab: 'lab', clerical: 'clerical', research: 'research' };
    const dbRole = roleMap[role];
    if (!dbRole) return res.status(400).json({ message: 'Invalid self-registration role' });

    // Map to staff role enum
    const staffRoleMap = { jmo: 'JMO', doctor: 'Doctor', lab: 'Laboratory Staff', clerical: 'Clerical Officer', research: 'Research User' };
